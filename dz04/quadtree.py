...
class QuadTree(object):
    ...
    def __init__(self, items=None, depth=4, bounding_rect=None):
        """Creates a quad-tree.
        ...
        if bounding_rect:
            bounding_rect = Rectangle(bounding_rect)
        else:
            # If there isn't a bounding rect, then calculate it from the items.
            if items:
                bounding_rect = Rectangle(items[0].get_bounding_rectangle())
                for item in items[1:]:
                    bounding_rect.add(Rectangle(item.get_bounding_rectangle()))
            else:
                # in case there are no items, assume a big rect (100x100 meters)
                bounding_rect = Rectangle((0.0, 0.0, 100.0, 100.0))
    ...
    def insert_items(self, items):
        ...
        rect_items = [(item, Rectangle(item.get_bounding_rectangle()))
                      for item in items]
